...
> **Note:** This documentation is for the legacy v2.x API. For the current API, see the [main documentation](./README.md).
...
function App() {
  ...
  return (
    <div>
      <Scrollbars ref={scrollbarsRef} style={{ width: 500, height: 300 }}>
        {/* your content */}
      </Scrollbars>
...
function CustomScrollbars({ children, ...props }) {
  const handleScroll = (event, values) => {
    console.log(values);
    /*
    {
        left: 0,
  ...
  return (
    <Scrollbars onScroll={handleScroll} {...props}>
      {children}
    </Scrollbars>
  );
}
